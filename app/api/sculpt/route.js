...
export async function POST(req) {
  try {
    ...
    // sculpt requires render3d
    const render3d = await latestAssetByKind(supabase, gallery_id, 'render3d')
    if (!render3d?.url) {
      return NextResponse.json({ error: 'No render3d asset found for this gallery_id' }, { status: 400 })
    }

    const outUrl = render3d.url.includes('?')
      ? `${render3d.url}&v=${Date.now()}`
      : `${render3d.url}?v=${Date.now()}`
    ...
    return NextResponse.json({ error: e?.message || 'sculpt failed' }, { status: 500 })
  }
}
