...
export async function POST(req) {
  try {
    ...
    if (!refined?.url) return NextResponse.json({ error: 'No refined asset found' }, { status: 400 })

    const outUrl = refined.url.includes('?')
      ? `${refined.url}&redo=${Date.now()}`
      : `${refined.url}?redo=${Date.now()}`
    ...
    return NextResponse.json({ error: e?.message || 'redo_render3d failed' }, { status: 500 })
  }
}
